...
export async function sync(params = {}) {
  ...
  let query = supabase
    .from('marketplace_connections')
    .select('id, shop_id, shop_name, access_token, partner_id, status')
  ...
  const { data: connections, error } = await query.limit(1)
  ...
  // Shop API signature: partner_id + path + timestamp + access_token + shop_id
  const baseStr = `${PARTNER_ID}${path}${timestamp}${access_token}${shop_id}`
  const sign = crypto.createHmac('sha256', PARTNER_KEY).update(baseStr).digest('hex')
  ...
  if (apiResponse.error && apiResponse.error !== '') {
    throw new Error(
      apiResponse.message || apiResponse.error || 'Shopee get_shop_info failed.'
    )
  }
