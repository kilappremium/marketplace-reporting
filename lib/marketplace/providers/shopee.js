...
function resolveUnixRange(dateStart, dateEnd) {
  if (!dateStart && !dateEnd) {
    return yesterdayToTodayUnixRange()
  }
...
export async function sync(params = {}) {
  ...
  const supabase = getSupabaseAdmin()
  const { dateStart, dateEnd, connectionId } = params
  ...
  const { updatedRows, insertedRows } = await savePenjualanHarian(supabase, dailyRows)
