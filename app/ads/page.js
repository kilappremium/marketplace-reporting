...
Berikan: ringkasan performa iklan, temuan penting (ROI/ROAS, efisiensi biaya, konversi), dan 2 rekomendasi konkret yang bisa langsung dieksekusi tim marketing.`
  ...
  function buildKonteksAds() {
    const cards = summaryCards[activeTab] || []
    const dataRingkas = cards.map(c => `${c.label}: ${c.value}`).join(', ')
    const platformLabel = PLATFORM_CONFIG[activeTab].label
    return `Kamu adalah AI analyst iklan digital untuk platform marketplace Kilap.
Kamu memiliki data performa iklan berikut dan harus menjawab berdasarkan data ini.

Platform aktif: ${platformLabel}
Periode: ${filterStart} s/d ${filterEnd}
${activeTab === 'shopee' && filterJenisShopee !== 'semua' ? `Filter: ${filterJenisShopee}` : ''}
...
${anomaliAds.length > 0 ? `ANOMALI:\n${anomaliAds.map(a => '- ' + a).join('\n')}` : 'Tidak ada anomali.'}
...
Jawab dalam Bahasa Indonesia yang natural. Gunakan angka dari data di atas saat menjawab. Jika ditanya di luar data yang tersedia, katakan data tersebut tidak tersedia.`
  ...
  async function sendChat() {
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { role: 'assistant', content: 'Maaf, terjadi kesalahan. Silakan coba lagi.', error: true }])
    }
      ...
      <div style={{ background:'#fff', border:'1px solid #FFE0CC', borderRadius:12, marginBottom:20, boxShadow:'0 2px 8px rgba(255,100,0,0.06)', overflow:'hidden' }}>
        ...
        <div onClick={() => setChatOpen(o => !o)}
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <div style={{ width:32, height:32, borderRadius:8, background:'linear-gradient(135deg,#FF6B35,#FF8C00)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:16 }}>💬</div>
            <div>
              <p style={{ margin:0, fontWeight:700, fontSize:14, color:'#1A1A1A' }}>Tanya AI tentang data {PLATFORM_CONFIG[activeTab].label}</p>
              <p style={{ margin:0, fontSize:11, color:'#999' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            ...
            <span style={{ fontSize:18, color:'#FF6B35', fontWeight:300, transition:'transform 0.2s', display:'inline-block', transform: chatOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}>⌄</span>
          </div>
        </div>
          ...
          <div>
              ...
              {chatMessages.length === 0 && (
                <div style={{ textAlign:'center', padding:'16px 0' }}>
                  ...
                  <div style={{ display:'flex', flexWrap:'wrap', gap:8, justifyContent:'center' }}>
                    {[
                      activeTab === 'shopee' ? 'Kenapa CTR Iklan Produk rendah?' : activeTab === 'tiktok' ? 'Bagaimana efisiensi budget TikTok?' : 'Analisis CPM Meta bulan ini',
                      'Apa penyebab ROI turun?',
                      'Campaign mana yang paling efisien?',
                      'Rekomendasi untuk meningkatkan konversi',
                      'Bandingkan performa vs periode lalu',
                    ].map(q => (
            ...
            <div style={{ padding:'12px 20px', borderTop:'1px solid #FFE0CC', background:'#FAFAFA', display:'flex', gap:10, alignItems:'flex-end' }}>
              <textarea
                ...
                placeholder="Tanya tentang performa iklan... (Enter untuk kirim)"
                rows={2}
